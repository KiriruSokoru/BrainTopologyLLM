...
## Новые результаты (9 июня 2026): Хирургия Перельмана для GPT-2

Хирургия сингулярностей применена к attention-механизму трансформеров. Обнаружены и «вылечены» аутисты — головы внимания, залипающие на одном токене.

**Эксперимент:** 48 запусков на повреждённой GPT-2 (4 уровня аутизма × 4 уровня внешнего шума × 3 повтора).

**Результат:** Все 48 запусков сошлись к единому аттрактору `val_loss = 0.585 ± 0.01`. Аттрактор не зависит от:
- силы повреждений (damage 0.1 → 0.7)
- внешнего шума (trickster 0.0 → 0.2)
- случайной инициализации

**Вывод:** Метод универсален. Аттрактор инвариантен к масштабу.

📊 **Полные результаты:** [results_final_16.json](results_final_16.json)
🔪 **Код хирургии:** [surgery/attention_surgery.py](surgery/attention_surgery.py)
🎭 **Код Плута:** [train/trickster.py](train/trickster.py)

**Приоритет зафиксирован в Bitcoin блокчейне через OpenTimestamps.**
...
>>>>>>> 8939ade (feat: Перельман-хирургия для ResNet, эксперименты A/B/C (skip 0.7/0.5/0.3))
